...
<section aria-labelledby="card-beranda" class="card-beranda">
    ...
    <a href="{{ route('layanan-surat-riwayat') }}" class="card">
        <h2>{{$suratMasukBulanIni}}</h2>
        <p>Surat yang masuk bulan ini</p>
    </a>
    <a href="{{ route('data-warga') }}" class="card">
        <h2>{{$warga}}</h2>
        @hasanyrole('admin|kades')
        <p>Populasi saat ini di <strong>Desa Rawapanjang</strong>: {{ $warga }} jiwa</p>
        @endhasanyrole

        @role('rt')
        <p>Populasi saat ini di <strong>{{ $user->akses }}</strong>: {{ $warga }} jiwa</p>
        @endrole
