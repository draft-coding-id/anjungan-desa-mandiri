...
<x-info-box :message="'Silakan lengkapi data berikut untuk pembuatan Surat Permohonan Perubahan Kartu Keluarga.'" />

<form action="{{ route('submitForm') }}" method="POST" enctype="multipart/form-data">
  @csrf
  <input type="hidden" name="warga_id" value="{{ $warga->id }}">
  <input type="hidden" name="jenis_surat" value="SPPKK">
  <input type="hidden" name="keperluan" value="Permohonan Kartu Keluarga baru WNI">
